...
class JSONPreferencesKeeper:
    ...
    def serilaization_functions(self) -> dict[any, tuple[Callable, Callable]]:
        ...
        """
        return {
            # tkinter Variable instances have special .set() method to assing values.
            # So we just need an instance iself and call this set method with stored
            # value as an argument
            tkinter.Variable: (lambda p: p.get(), lambda _, obj, val: obj.set(val)),

            # PathLike objects are stored as full path strings and before setting up,
            # they're converted back to Path objects
